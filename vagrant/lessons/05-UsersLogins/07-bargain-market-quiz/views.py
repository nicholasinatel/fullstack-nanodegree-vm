...
redis = Redis() # Access Redis Instace for Rate Limiting
...
class RateLimit(object):
    expiration_window = 10 # 10 seconds for expirate in redis for my key

    def __init__(self, key_prefix, limit, per, send_x_headers):
        # When can the timestamp reset itself
        self.reset = (int(time.time()) // per) * per + per
        self.key = key_prefix + str(self.reset)
        self.limit = limit
        self.per = per
        self.send_x_headers = send_x_headers
        # in case of exceptions use pipeline
        p = redis.pipeline()
        p.incr(self.key)
        p.expireat(self.key, self.reset + self.expiration_window)
        self.current = min(p.execute()[0], limit)
    
    # Counting remaining requests and over limit
